...
Based on   https://github.com/CherifiImene/buildings_and_road_segmentation
"""
...
def showSegData(dl):
    ...
    for i, b in enumerate(dl):
        ...
        fn, img, mask = b["img_name"], b["image"], b["mask"]
        #print(img, mask, img.shape, mask.shape)
        sample = np.random.choice(len(img), 1, replace=False)[0]
       
        x = dut.tensor2Img(img[sample])
        y = dut.tensor2Img(mask[sample])
        #mask = msk[0].unsqueeze(0)
        print(f"img shape: {img[sample].shape}, mask shape:{mask[sample].shape}, image name:{fn[sample]}")
   
        #x.show()
        showImgSegMask(x, y)
        #showImageWithRGBMask(x, y, labeler)
...
def showMultiClassSegData(dl, ml):
    ...
    for i, b in enumerate(dl):
        ...
        x = np.array(x, dtype=np.float32)
        print(f"img shape: {img[0].shape}, {img[0].min()}, {img[0].max()}, mask shape:{msk[0].shape}, image name:{fn[0]}")
        #y = dut.tensor2SegMap(msk[0])
 
        y = msk[0].numpy()
        if y.ndim == 3: #for BCELogitsloss, the mask has 3 dim
            y = np.transpose(y, (1,2,0))
            print("Multiclass", y.shape, y.min(), y.max())
        else:    #for CrossEntropyloss, the mask has 2 dim
            y = ml.classToRGBMaskComplex(msk[0])
        showImgSegMask(x, y)
        return
