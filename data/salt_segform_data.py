...
class SaltDataset(Dataset):
    """ Dataset processor for Salt dataset """
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_paths is not None:
            ...
            #mask = Image.open(self.mask_paths[idx]).convert("L") #.convert("L")
            mask = cv2.imread(self.mask_paths[idx], cv2.IMREAD_GRAYSCALE)
            #print(self.mask_paths[idx])
            #print(self.mask_paths[idx], mask.shape, img[50, 50, :], mask[50, 50])
...
def loadTrainingData(cfg, labeler, feature_extractor):
    ...
    train_ds = SaltDataset(feature_extractor, train_imgs, train_masks, labeler)
    #get the validation dataset
    val_ds = SaltDataset(feature_extractor, val_imgs, val_masks, labeler)
...
def loadTestData(cfg, labeler, feature_extractor, gt_mask=False):
    ...
    #test data 
    test_img_paths = sorted(os.listdir(cfg.test_path))[:20]
    ...
    for idx, im in enumerate(test_img_paths):
        ...
        if gt_mask: #if GT mask labels exist for the test data 
            mask_path = os.path.join(cfg.test_mask_path, fn + ".png")
            test_mask_paths.append(mask_path) 
    
    print("number of test images", len(test_img_paths))
    #get the test dataset
    test_ds = SaltDataset(feature_extractor, test_img_paths, test_mask_paths, labeler)
